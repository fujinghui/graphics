...
int WINAPI WinMain(_In_ HINSTANCE hInstance, _In_opt_ HINSTANCE hPrevInstance, _In_ LPSTR lpCmdLine, _In_ int nShowCmd){
	...
	hwnd = CreateWindow(scClassName, scTitle, WS_CAPTION |
		WS_SYSMENU |
		WS_THICKFRAME |
		WS_MINIMIZEBOX, 100, 100, SCREEN_WIDTH, SCREEN_HEIGHT, NULL, NULL, hInstance, NULL);
	if (hwnd == NULL)
		return 0;
...
void Game_Init(HWND hwnd){
	...
	Load_OBJECT4DV2_PLG(&object[0], "tank2_8b.plg", &vscale, &vpos, &vrot, 0);
	vpos.x = 0; vpos.z = -50; vpos.y = 0;
	vscale.x = 10; vscale.y = 10; vscale.z = 10;
	Load_OBJECT4DV2_3DASC(&object[1], "cube01.asc", &vscale, &vpos, &vrot, 0);
	//将加载的第一个物体的数据给之后的每个物体数据
	//for (int i = 1; i < 10; i++)
	//{
		//memcpy(&object[i], &object[0], sizeof(OBJECT4DV1));
	//}
	vscale.x = 50; vscale.y = 50; vscale.z = 50;

	/*
	date:2016 10 11
	time:00:03
	error fun:Load_OBJECT4DV2_3DASC
	watting solve to femy
	*/
	//if (Load_OBJECT4DV2_3DASC(&object[9], "cube01.asc", &vscale, &vpos, &vrot, 0))
	//{
	//	MessageBox(NULL, "text", "title", MB_OK);
	//}
	//MessageBox(NULL, "Load Model Success", "PROMPT", MB_OK);
	for (int i = 0; i < 1; i++)
	{
...
void Game_Main(HDC hdc){
	...
	Build_XYZ_Rotation_MATRIX4X4(ang_y, ang_y, 0, &mrot);
	//缓慢地旋转y轴
	if (ang_y >= 360.0)
	{
		ang_y = 0.0f;
	...
	{
		Reset_OBJECT4DV2(&object[index]);
		if (!(Cull_OBJECT4DV2(&object[index], &cam, CULL_OBJECT_XYZ_PLANES)))
		{
			 
		}
		Transform_OBJECT4DV2(&object[index], &mrot, TRANSFORM_LOCAL_TO_TRANS, 1);
		Model_To_World_OBJECT4DV2(&object[index], TRANSFORM_TRANS_ONLY);
		Remove_Backfaces_OBJECT4DV2(&object[index], &cam);
		if (light_on == 1)// && index != 9)
		{
			Light_OBJECT4DV2_World16(&object[index], &cam, 4);
			TextOut(hdc, 10, 180, "active light", strlen("active light"));
		}
		World_To_Camera_OBJECT4DV2(&object[index], &cam);
		Camera_To_Perspective_OBJECT4DV2(&object[index], &cam);
		Perspective_To_Screen_OBJECT4DV2(&object[index], &cam);
		if (w_state)
			Draw_OBJECT4DV12_Solid16(&object[index], hdc, 1);
		else
			Draw_OBJECT4DV2_Wire16(&object[index], hdc, 1);
	}
	...
	x1 = rand() % 500; y1 = rand() % 500;
	x2 = rand() % 500; y2 = rand() % 500;
	x3 = rand() % 500; y3 = rand() % 500;
	
	Draw_Gouraud_Trangle(399, 290, (float)0x999900, 398, 309, (float)0x999900, 379, 308, (float)0x999900, hdc, 1);

	//Draw_Gouraud_Trangle(x1, y1, (float)0x888888, x2, y2, (float)0x888888, x3, y3, (float)0x888888, hdc, 1);
	
	//Draw_Gouraud_Trangle(280, 300, (float)0x0000ff, 400, 300, (float)0x00ff00, 400, 250, (float)0xff0000, hdc, 1);
	//Draw_Gouraud_Trangle(400, 200, (float)0xff0000, 300, 300, (float)0x0000ff, 400, 400, (float)0x00ff00, hdc, 1);
	//Draw_Gouraud_Trangle(100, 100, (float)0x999999, 0, 270, (float)0x00ff00, 200, 270, (float)0x0000ff, hdc, 1);
