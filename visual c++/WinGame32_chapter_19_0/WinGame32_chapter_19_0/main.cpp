...
D3DMATERIAL9* g_pMaterils = NULL;
LPD3DXMESH g_pMesh = NULL;


D3DMATERIAL9 g_MaterialsWall;
LPD3DXMESH g_MeshWall;
...
int WINAPI WinMain(_In_ HINSTANCE hInstance, _In_opt_ HINSTANCE hPrevInstance, _In_ LPSTR lpCmdLine, _In_ int nShowCmd){
	...
	g_pDInput->Init(femy.hWnd, hInstance, DISCL_FOREGROUND | DISCL_NONEXCLUSIVE, DISCL_FOREGROUND | DISCL_NONEXCLUSIVE);
...
HRESULT Direct3D_Init(HWND hwnd){
	...
	d3dpp.AutoDepthStencilFormat = D3DFMT_D24S8;			//每个像素24为深度值
	d3dpp.Flags = 0;
...
//	g_pd3dDevice->SetRenderState(D3DRS_ZWRITEENABLE, true);		//更新深度缓冲区
...
HRESULT Objects_Init(HWND hwnd){
	...
	//创建一面墙
	D3DXCreateBox(g_pd3dDevice, 30.0f, 30.0f, 0.5f, &g_MeshWall, NULL);
	g_MaterialsWall.Ambient = D3DXCOLOR(0.4f, 0.4f, 0.1f, 1.0f);				//环境光
	g_MaterialsWall.Diffuse = D3DXCOLOR(0.6f, 0.6f, 0.4f, 1.0f);				//漫反射
	g_MaterialsWall.Specular = D3DXCOLOR(0.1f, 0.1f, 0.1f, 1.0f);				//镜面反射
	...
	g_pMaterils = new D3DMATERIAL9[g_dwNumMtrls];
	g_pTextures = new LPDIRECT3DTEXTURE9[g_dwNumMtrls]; 
	for (DWORD i = 0; i < g_dwNumMtrls; i++)
	{
		g_pMaterils[i] = pMtrls[i].MatD3D;
		g_pMaterils[i].Ambient = g_pMaterils[i].Diffuse;
		
		g_pTextures[i] = NULL;
		D3DXCreateTextureFromFileA(g_pd3dDevice, pMtrls[i].pTextureFilename, &g_pTextures[i]);
	//	wsprintf(buffer, TEXT("%s\0"), pMtrls[i].pTextureFilename);
	//	MessageBox(NULL, buffer, TEXT("title"), MB_OK);
	}
	//MessageBox(NULL, TEXT("info"), TEXT("title"), MB_OK);
...
void Direct3D_Render(HWND hwnd){
	...
	for (DWORD i = 0; i < g_dwNumMtrls; i++)
	{
		g_pd3dDevice->SetMaterial(&g_pMaterils[i]);
...
void Light_Init(){
	...
	light.Ambient		= D3DXCOLOR(0.5f, 0.5f, 0.5f, 1.0f);			//环境光
	light.Diffuse		= D3DXCOLOR(1.0f, 1.0f, 1.0f, 1.0f);
	light.Specular		= D3DXCOLOR(0.1f, 0.1f, 0.1f, 1.0f);
	light.Direction = D3DXVECTOR3(1.0f, 0.0f, 1.0f);
	
	g_pd3dDevice->SetLight(0, &light);			//设置0光源
	g_pd3dDevice->LightEnable(0, true);			//开启0号光源
...
//	wsprintf(buffer, TEXT("%d\0"), g_dwNumMtrls);
//	MessageBox(NULL, buffer, TEXT("title"), MB_OK);
